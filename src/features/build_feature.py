...
def build_feature(df):
    ...
    df["clv"] = df["total_spend"] / df["tenure"]
    ...
    # 5. Engagement Score - combines usage and support interaction
    df["engagement_score"] = (df["usage_frequency"] + df["support_calls"]) / 2
    ...
    # 7. Value to Company - average spend per month
    df["value_to_company"] = df["total_spend"] / (df["tenure"] / 12)

    # 8. Risk Score - composite metric indicating churn risk
    df["risk_score"] = (
    ...
    # 9. tenure Category - binned tenure
    ...
    # 11. Spend Category - binned total spend
